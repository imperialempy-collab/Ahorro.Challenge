...
if (localEmail && localStatus) {
    if (localStatus === 'prueba') {
        ...
        if (ultimaAlerta !== hoy) {
            ...
            setTimeout(() => {
                window.interactuarApp('alert', '🎁 Modo Prueba', `Estás en tu día ${diffDays} de 7 de prueba gratis.`);
            }, 500);
...
// ==========================================
// MACRO GESTION LOGIC (Cuentas, Gastos, Reportes)
// ==========================================
...
// SOLUCIÓN AL BUG DE LOS ÍCONOS: Si no existe la imagen, crea un logo de 2 letras con colores.
window.renderizarSelectorIconos = () => { 
...
window.seleccionarIcono = (logo) => { 
    iconoSeleccionado = logo; 
    ...
    const elementId = 'logo-' + logo.replace('.png', ''); 
    const seleccionado = document.getElementById(elementId); 
    if (seleccionado) { 
        seleccionado.classList.remove('border-slate-100'); 
        seleccionado.classList.add('ring-2', 'ring-primary', 'border-primary'); 
        seleccionado.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); 
    } 
};
...
// SOLUCIÓN AL BUG DE ACEPTAR EN ALERTA: Asignamos el click directo para que no muera esperando la carga.
const btnCancel = document.getElementById('btnModalCancelar');
const btnConfirm = document.getElementById('btnModalConfirmar');
if(btnCancel) btnCancel.onclick = () => window.cerrarModalAccion(null); 
if(btnConfirm) btnConfirm.onclick = () => { 
    const inputContenedor = document.getElementById('modalAccionInputContenedor'); 
    if (!inputContenedor.classList.contains('hidden')) { 
        window.cerrarModalAccion(document.getElementById('modalAccionInput').value); 
    } else { 
        window.cerrarModalAccion(true); 
    } 
};
...
// SOLUCIÓN AL BUG DE LAS CUENTAS CLONADAS: Frenamos el multi-click y cerramos el modal instantáneamente.
let isSaving = false;
window.guardarFormulario = () => { 
    ...
    if(!v1 || !v3 || isNaN(v3)) { window.interactuarApp('alert', 'Datos incompletos', 'Por favor completá al menos el nombre y el monto para guardar.'); return; } 
