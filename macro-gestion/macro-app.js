        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (!hasLocalMacro && userData.macro_data) {
                    ...
                    cuentas = JSON.parse(localStorage.getItem('mg_cuentas')) || [];
                    gastos = JSON.parse(localStorage.getItem('mg_gastos')) || [];
                    historialMovimientos = JSON.parse(localStorage.getItem('mg_historial')) || [];
...
// =========================================================================
// SISTEMA ANTIFALLOS PARA ÍCONOS (A prueba de balas)
// =========================================================================
window.iconoFallo = (imgElement, nombreOriginal) => {
    imgElement.onerror = null; // Evita bucles infinitos
    let nombreCorto = '🏦';
    if (nombreOriginal && nombreOriginal.includes('.png')) {
        nombreCorto = nombreOriginal.replace('.png', '').substring(0, 2).toUpperCase();
    }
    const parent = imgElement.parentElement;
    if(parent) {
        parent.innerHTML = `<span class="text-lg font-black text-primary tracking-widest">${nombreCorto}</span>`;
    }
};
    ...
    if(grilla){ 
        grilla.innerHTML = logosDisponibles.map(logo => { 
            ...
            return `<div id="logo-${logoId}" onclick="seleccionarIcono('${logo}')" class="logo-opt w-14 h-14 shrink-0 bg-white border-2 border-slate-100 rounded-xl p-0.5 cursor-pointer hover:bg-slate-50 transition-all flex items-center justify-center shadow-sm overflow-hidden">
                        <img src="../logos/${logo}?v=2" onerror="window.iconoFallo(this, '${logo}')" class="w-full h-full object-contain scale-125 transform transition-transform" alt="${logo}">
                    </div>`; 
        }).join(''); 
    }
};
...
window.renderizarCuentas = () => { 
    ...
    cuentas.forEach(c => { 
        let iconoHtml = (c.icono && c.icono.includes('.png')) 
            ? `<img src="../logos/${c.icono}?v=2" onerror="window.iconoFallo(this, '${c.icono}')" class="w-full h-full object-contain scale-125 transform" alt="${c.nombre}">` 
